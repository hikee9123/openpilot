...
class HudRenderer(Widget):
  ...
  def _draw_speed_camera_alert(self, rect: rl.Rectangle) -> None:
    ...
    label_text, label_font_size, label_size = self._fit_text(info_label, width, 18, 14)
    rl.draw_text_ex(
      self._font_medium,
      label_text,
      rl.Vector2(x + (width - label_size.x) / 2, y + 4),
      label_font_size,
      0,
      COLORS.SPEED_CAMERA,
    )
  ...
  def _draw_camera_direction_pointer(self, center_x: float, center_y: float, radius: int) -> None:
    ...
    pointer_length = max(16.0, radius * 0.38)
    pointer_half_width = max(9.0, radius * 0.18)
